  ...
  while (inQueue){
    ...
    try 
    {
      finalMatchID = await runTransaction(db, async (transaction) => {
        ...
        if (clientUserMatchedID !== "Open"){ //the error im trying to debug occurs when chosen by someone
        ...
        for (const doc of matchMakingPoolSnap.docs) 
        {
          ...
          if (curUser.exists() && curUser.id !== clientUserID && curUser.id !== "emptyQ" && (curUser.data().matchedID === "Open" || curUser.data().matchedID === clientUserID)) 
          { 
            matchedUser = curUser;
            break;
          }
        ...
        if (matchedUser.data().matchedID === "Open"){
          transaction.update(matchedUser.ref, { matchedID: writetoMatch});console.log("error is here")
    ...
    try
    {
      ...
      const matchUserDocRefCheck: DocumentReference<DocumentData, DocumentData> = doc(db,'queue',finalMatchID);
      const matchUserDocSnapCheck: DocumentSnapshot<DocumentData, DocumentData> = await getDoc(matchUserDocRefCheck);
