import { createUserWithEmailAndPassword, auth, doc, db, setDoc, getDoc, ref, getDownloadURL, storage} from "../firebase"
...
const RegisterScreen = () => {
    ...
    const handleRegister = async (email, password) => {
        ...
        try {
            ...
            await setDoc(userCreds[1], {email: auth.currentUser.email, jilt: true, rating: 0, matchedID: "None", inventory: [{}], new: true}, {merge: true})
        } catch (error) {
            alert(error.message);
        }
    }
    ...
    return (        <View style = {{flex:1, flexDirection:'column', backgroundColor: 'rgba(28,29,35,255)'}}>
...
<AnimateIcon onPress = {()=> setActive('C')} iconComponent={<Image source={{uri:starterC}} resizeMode ='contain' style={{ backgroundColor:'rgba(28,29,35,.25)', padding:25, width: 75, height: 75,  borderWidth: active === 'C' ? 6 : 2, 
        ...
        <View style = {{flex:1, margin: 20, marginTop: 40}}><FlashButton pressFunc={()=>handleRegister(email, password) } text={"Register"} ></FlashButton>
